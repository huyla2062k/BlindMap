...
class MainViewModel(application: Application) : AndroidViewModel(application), TextToSpeech.OnInitListener {
    ...
    fun handleSpeechResult(data: Intent?) {
        ...
        if (!results.isNullOrEmpty()) {
            ...
            Log.d(TAG, "handleSpeechResult: $spokenText, state: $speechState at 08:11 AM +07, 02/10/2025")
            when (speechState) {
    ...
    fun getCoordinatesFromAddress(address: String) {
        ...
        Log.d(TAG, "getCoordinatesFromAddress: $encodedAddress at 08:11 AM +07, 02/10/2025")
        val url = "https://maps.track-asia.com/api/v2/geocode/json?address=$encodedAddress&key=public_key"
        val request = Request.Builder().url(url).build()
        client.newCall(request).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                Log.e(TAG, "Failed to get coordinates: ${e.message} at 08:11 AM +07, 02/10/2025")
            ...
            override fun onResponse(call: Call, response: Response) {
                response.body?.string()?.let { json ->
                    ...
                    if (jsonObject.getString("status") == "OK") {
                        ...
                        Log.d(TAG, "onResponse: lat/lng: (${destination?.latitude},${destination?.longitude}) at 08:11 AM +07, 02/10/2025")
    ...
    fun getDirections(origin: LatLng, destination: LatLng) {
        ...
        Log.d(TAG, "getDirections: $url at 08:11 AM +07, 02/10/2025")
        val request = Request.Builder().url(url).build()
        client.newCall(request).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                Log.e(TAG, "Failed to get directions: ${e.message} at 08:11 AM +07, 02/10/2025")
            ...
            override fun onResponse(call: Call, response: Response) {
                response.body?.string()?.let { jsonString ->
                    ...
                    if (json.getString("status") == "OK") {
                        ...
                        currentStepIndex = 0
                        Log.d(TAG, "Navigation steps loaded: ${navigationSteps?.length()} steps at 08:11 AM +07, 02/10/2025")
            ...
            if (steps.length() > 0 && currentStepIndex < steps.length()) {
                ...
                for (i in currentStepIndex until steps.length()) {
                    ...
                    if (!startNewSegment && isLocationClose(currentPosition, startLocation, 10.0)) {
                        ...
                        currentStepIndex = i
                        Log.d(TAG, "Starting new segment at step $currentStepIndex at 08:11 AM +07, 02/10/2025")
                    }
                    if (startNewSegment) {
                ...
                if (remainingPoints.isNotEmpty()) {
                    ...
                    Log.d(TAG, "Reached or passed last step, clearing map at 08:11 AM +07, 02/10/2025")
                    _mapUpdate.postValue(MapUpdate(clearMap = true)) // Xóa khi đến đích
                } else {
                    Log.w(TAG, "Warning: remainingPoints is empty but not at destination at 08:11 AM +07, 02/10/2025")
                }
            } else {
                Log.d(TAG, "No steps or index out of bounds, clearing map at 08:11 AM +07, 02/10/2025")
        ...
        navigationSteps?.let { steps ->
            if (currentStepIndex < steps.length()) {
                ...
                if (distanceToEnd[0] < 10) { // Chuyển bước khi đến gần điểm kết thúc
                    currentStepIndex++
                    Log.d(TAG, "Moved to step $currentStepIndex at 08:11 AM +07, 02/10/2025")
                    announceCurrentStep()
                } else if (distanceToStart(currentLatLng, startLocation) > 20 && currentStepIndex > 0) { // Quay lại bước trước
                    currentStepIndex--
                    Log.d(TAG, "Reverted to step $currentStepIndex at 08:11 AM +07, 02/10/2025")
                    announceCurrentStep()
                }
            } else {
                Log.d(TAG, "Current step index ($currentStepIndex) exceeds steps length (${steps.length()}) at 08:11 AM +07, 02/10/2025")
            }
        }
    }
    ...
    private fun announceDetailedTurn(currentPosition: LatLng, step: JSONObject) {
        ...
        if (roundedDistance > 0 && roundedDistance != lastAnnouncedDistance) {
            ...
            val distanceText = roundedDistance.toString()
        ...
        if (!isCameraStarted) {
            ...
            cameraProviderFuture.addListener({
                ...
                try {
                    ...
                    Log.d(TAG, "Camera started at 08:11 AM +07, 02/10/2025")
                } catch (exc: Exception) {
                    Log.e(TAG, "Camera failed to start: ${exc.message} at 08:11 AM +07, 02/10/2025")
    ...
    fun startLocationUpdates() {
        if (checkLocationPermission(getApplication())) {
            ...
            Log.d(TAG, "Location updates started at 08:11 AM +07, 02/10/2025")
        }
    }
        ...
        if (checkLocationPermission(getApplication())) {
            fusedLocationClient.lastLocation.addOnSuccessListener { location: Location? ->
                location?.let {
                    ...
                    Log.d(TAG, "Current location: ${it.latitude}, ${it.longitude} at 08:11 AM +07, 02/10/2025")
                }
            }
        }
    }
    ...
    fun stopCamera() {
        if (isCameraStarted) {
            ...
            Log.d(TAG, "Camera stopped at 08:11 AM +07, 02/10/2025")
        }
    }
    ...
    fun ttTranslation(message: String) {
        ...
        Log.d(TAG, "TTS speaking: $message at 08:11 AM +07, 02/10/2025")
    }
    ...
    override fun onCleared() {
        ...
        stopCamera()
        Log.d(TAG, "ViewModel cleared at 08:11 AM +07, 02/10/2025")
        super.onCleared()
    }
}